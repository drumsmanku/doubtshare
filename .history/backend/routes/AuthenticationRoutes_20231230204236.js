...
router.post('/register/student', async (req, res) => {
  ...
  try {
    ...
    client.query(newUserQuery).then(()=>{
     ...
     res.json({ message: 'User registered successfully!' , token, name:newUserQuery[0].name});
    })
...
router.post('/register/tutor', async (req, res) => {
  ...
  try {
    ...
    const newUserQuery = {
      text: 'INSERT INTO tutors(name, email, password) VALUES($1, $2, $3)',
      values: [name, email, hashedPassword],
    };

    client.query(newUserQuery).then(()=>{
      const token = jwt.sign({ email:email }, process.env.JWT_SECRET_KEY, {expiresIn: 3600 });
     res.json({ message: 'User registered successfully!' , token, name:newUserQuery.name});
    })
...
router.post('/login/tutor', async (req, res) => {
  ...
  const findUserQuery = {
    text: 'SELECT * FROM tutors WHERE email = $1',
    values: [email],
  };
