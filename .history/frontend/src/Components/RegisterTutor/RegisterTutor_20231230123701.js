...
function RegisterTutor() {
  ...
  const signup = (event) => {
    ...
    axios.post('http://localhost:4000/register/tutor', user)
      .then(res => {
    ...
    <div className={styles.container}>
      ...
      <form className={styles.form}>
        ...
        <div className={styles.inputContainer}>
          <h2 style={{alignSelf:'start', paddingLeft:'2.2rem', fontWeight:600}}>Create Account(Tutors)</h2>
          <div style={DivStyles}>
        ...
        <div style={{width:'100%', display:'flex', alignItems:'center', paddingLeft:'2rem'}}>
             <p>Are you a Student?</p>
             <u style={{ cursor:'pointer', marginLeft:'0.5rem'}} onClick={()=>{navigate('/register/student')}}>click here</u> 
          </div>
      ...
      <span style={{fontWeight:'bold', marginTop:'1rem'}}>Already have an account?<button style={{background:'none', color:'#36416A', fontSize:'medium', marginLeft:0, padding:0, width:'4rem', cursor:'pointer', border:'none', textDecoration:'underline'}} onClick={()=>{navigate('/login/tutor')}}>Sign in</button></span>
